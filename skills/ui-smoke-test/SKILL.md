...
- For a project-progress batch, select its parent only once. The first project-progress action expands the project-list search criteria and queries lifecycle statuses in order: `项目决策=20`, `项目实施=30`, `项目竣工=40`, `已终止=100`. Scope the shared exact-text click helper to the visible `.search-more` container, not to its button child, because the helper resolves the descendant command itself. Selecting a status already emits the source-confirmed `POST /fi-service/projInfo/listPage`; do not click 查询 again in the normal path. Accept the first non-empty result only when the captured request's `projStatus` equals that status's exact code, HTTP and JSON business status both succeed, the rendered card count/order and available display fields match the returned record collection, every card exposes the requested status, and the response's first `projId` equals the ID in the opened detail URL. Do not require the post-filter text snapshot to differ from the pre-filter snapshot: an already equivalent visible page is valid when it matches the captured response. A late unfiltered, other-status, or response-mismatched card resets rendered-list stability and can never become the parent. If the captured response and rendered cards still cannot align within the bounded wait, reissue the current filter once through the page-level 查询 action, capture its exact-status response, and repeat the same checks; never retry without a bound or accept the mismatched DOM. Cache the verified ID and detail URL and reuse that exact parent for every later Add/Edit/Delete command, including separate pytest subprocesses. Do not call `projProgress/preAdd` while selecting or replacing the parent, and do not provision a parent when every status query is empty; raise the single explicit no-matching-project precondition instead. The page's actual Add click retains its native `preAdd` behavior. If that UI action is blocked, report the block on the fixed project and never switch parent mid-batch. When an Edit/Delete action has no usable row-level action in the fixed parent's child table, create one child record through that same parent's Add form, return to the saved parent URL, and retry exactly once.
- Project-progress status-query exhaustion is a failed parent precondition, not a skipped operation. Let the discovery command fail once so its dependent validation commands are omitted for that run; do not turn the absence of a matching project into per-case skips.
...
- A persisted-parent page jump must not rely on one framework-specific `aria-label`. After entering the target page, confirm either the rendered active-page number or a changed parent-row snapshot, then wait for the chosen collection to stabilize. Keep unit coverage for both a missing active marker with changed rows and a direct jump where only the active-page number proves navigation.
...
- 页签子模块执行前读取 `EI_PAGE_TAB`。用可见的 `[role=tab]` 或 Element Plus `.el-tabs__item` 精确匹配页签文本；已激活时不重复点击，未激活时点击后必须确认 `aria-selected=true` 或 `is-active`。源码确认的页签未渲染、不可激活均为页面/部署契约失败，不能静默退回父页面或跳过。
...
- Preflight probes may reuse the saved browser-session credentials only for the exact target host, including its front-end-compatible authorization, cookies, and tenant header, and must never emit any of those values or storage-state content to run logs, Allure, or diagnostics. A matching HTTP 404 blocks only its dependent commands as `environment-version-mismatch`; network/TLS probe failures remain preflight evidence, not an automatic product defect.
- A concrete HTTP 404 blocks only the commands that depend on that probe, records an environment-preflight report, and must not be reclassified as a product defect. Other status codes remain ordinary execution evidence unless the manifest explicitly opts a 5xx status into a named capability failure; that exception blocks only the exact dependent action as `environment-capability-unavailable`, never as a source/deployment version mismatch. Compare an optional deployed-version header with the selected source revision and warn only after a configured persistent mismatch period.
- The launcher runs this preflight only after a successful source sync and before every pytest collection command, then writes `artifacts/runs/environment-api-preflight.json` and appends its blocked/warning counts to the source-sync log. When any logical targets are blocked, publish the blocked and still-runnable target counts to launcher status before collection starts.
...
- A chained preflight may extract a configured scalar such as an existing business ID only from a successful bounded JSON response and substitute it into a later read-only path. A missing extraction leaves the capability inconclusive and must not call the dependent step or fabricate a blocker. Keep response bodies out of the report.
- Scope every preflight conclusion to the operation it actually exercised. A successful list/detail read, even when it returns a non-empty child array, proves only that read path; it cannot establish downstream delete/insert/save permissions, constraints, or triggers and must not be named or reported as write-capability evidence.
- Cover the probe manifest's read-only POST guard, chained ID extraction, per-command 404 blocking, undeclared non-404 pass-through, explicitly configured capability 5xx, exact action-path matching, configured request payload, and delayed version-mismatch warning in `tests/test_environment_api.py`.
- Probe matching must accept either the selected real form code or normalized component path and, when `actionPaths` is declared, the full exact operation path. Missing probe configuration means no preflight target, not a failed test; malformed configuration must block before tests are scheduled. For grouped `EI_ACTIONS_JSON`, evaluate and filter each logical action independently, rewrite the batch with the survivors, and start no pytest command only when every action is blocked; one dependent action must never discard unrelated actions in the same browser batch.
...
- Authentication, TLS, network, and undeclared non-404 HTTP failures are not deployment evidence. Preserve their normal test behavior rather than reclassifying them as environment mismatch. A manifest may declare only concrete 5xx statuses as a named capability failure, and that result remains a distinct capability classification scoped to its exact form/component and action path.
...
- For a source-declared detail tree using `getUserFuncPermTree`, scan both `src/views` and `srcEi/views` in stable source-root and source-path order. Resolve a `fatherId` only when the call contains a literal or a referenced variable with an explicit source fallback. Feed the tree into runtime-menu conversion and require a unique deepest common component directory before attaching it to a page tab; a sibling tab must not inherit another tab's detail nodes.
...
- 同一删除专用命令中的参数化规则按逻辑操作身份共享详情和可删记录前置条件。首条规则在详情导航、可删记录查找或空表后的前置记录创建中发生断言或浏览器异常时，保留该条的原始失败；详情导航边界必须同时缓存普通断言和直接抛出的 Playwright `Error`/`TimeoutError`。同一操作后续删除规则必须以“删除共享前置条件未满足”跳过，不得再次尝试创建数据或重复相同失败。共用 URL/组件但逻辑操作身份不同的删除批次不得互相阻断。确认框、删除响应、取消效果和删除后终态等真实行为断言不属于共享前置，失败后不得阻断后续规则。每条规则仍保留独立 Allure 标题和用例 ID。

标准自动化先把已选嵌套 `operation_path` 归一为其外层表单上下文，再追加通用字段发现与验证；发现/验证可按页面、组件、表单码和外层动作去重。嵌套按钮的实际动作不能共享执行器：每条路径必须作为独立 pytest 参数项重新打开外层表单，只执行该条嵌套动作并完成自身保存和回读。禁止把多条路径合并进 `EI_ACTION_PATHS_JSON`，否则滚动、重渲染和失败归属会互相污染；也禁止在归一化之前直接丢弃嵌套选择，导致外层字段用例没有计划。

- 项目进度选择父项目时，状态筛选点击必须捕获同一次请求的业务成功响应，并确认渲染卡片与该响应逐项一致；只从第一个非空状态结果的首条记录进入详情。筛选响应为空、状态不一致、HTTP/业务失败或详情 ID 与首条响应不一致，均是共享前置失败，不能创建替代项目或退回未过滤列表。
...
- Before constructing the CRUD driver for each parameterized module action, set `EI_AUTOMATION_ACTION_SCOPE` to a stable hash of that action's module ID and action path. This supplements, rather than replaces, the launcher run ID and target sequence, so separately scheduled nested actions cannot submit the same generated parent identity.
...
- A dynamic child may declare an effective `maxLength` only as a positive integer. Loader coverage must assert the exact configured value and reject zero, negative, or nonnumeric values; pass the accepted value into the runtime child-field contract instead of relying on browser-side truncation or a module-specific Python branch.
- A dynamic child may declare a scalar `probeValue` as a manifest-owned conservative quick-probe input. Loader coverage must assert exact configured values and reject structured objects or arrays; standard automation ignores this override and retains its existing data-strategy and boundary-case semantics.
